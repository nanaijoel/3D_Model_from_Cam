...
"""
# === Meshing aus fused_points.ply ===========================================

def _estimate_normals_consistent(pcd):
  
    pts = np.asarray(pcd.points)
    rad = max(1e-4, 3.0 * _median_nn_distance(pts))
    try:
        pcd.estimate_normals(
            search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=float(rad), max_nn=30)
        )
...
def mesh_from_fused_points(
    mesh_dir: str,
    in_name: str = "fused_points.ply",
    out_name: str = "final_mesh.ply",
):

    # --- feste Parameter (bei Bedarf hier anpassen) ---
    MAX_PTS_TARGET   = 1_200_000   # Zielgröße fürs Voxel-Downsample
    POISSON_DEPTH    = 10          # Detailgrad (typisch 10..12)
    DENSITY_TRIM_Q   = 0.10        # unterstes Quantil entfernen (0..0.2 sinnvoll)
    SMOOTH_ITER      = 1           # 0..3
    POISSON_SCALE    = 1.2         # Poisson-Standard
    POISSON_LINEAR   = False

    in_ply  = os.path.join(mesh_dir, in_name)
    out_ply = os.path.join(mesh_dir, out_name)

    if not HAS_O3D:
        raise RuntimeError("Open3D ist nicht verfügbar – Meshing benötigt Open3D.")

    # 1) Punktwolke laden
    pcd = o3d.io.read_point_cloud(in_ply)
    if pcd is None or np.asarray(pcd.points).size == 0:
        raise RuntimeError(f"Leere oder fehlende Punktwolke: {in_ply}")

    # 1a) Vorab-Downsample auf Zielgröße (iterativ Voxel erhöhen)
    pts = np.asarray(pcd.points)
    n = pts.shape[0]
    if n > MAX_PTS_TARGET:
        voxel = max(1e-4, 0.6 * _median_nn_distance(pts))
        for _ in range(7):
            p_try = pcd.voxel_down_sample(voxel_size=float(voxel))
            if np.asarray(p_try.points).shape[0] <= MAX_PTS_TARGET:
                pcd = p_try
                break
            voxel *= 1.35

    # 2) Normals schätzen
    _estimate_normals_consistent(pcd)

    # 3) Poisson-Rekonstruktion (ohne BPA-Alternative)
    mesh, dens = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(
        pcd, depth=int(POISSON_DEPTH), scale=float(POISSON_SCALE), linear_fit=bool(POISSON_LINEAR)
    )

    # 3a) Dünne Bereiche per Dichte trimmen (robusteres Ergebnis)
    try:
        dens = np.asarray(dens)
        thr = np.quantile(dens, float(DENSITY_TRIM_Q))
        keep = dens >= thr
        mesh = mesh.select_by_index(np.where(keep)[0])
    except Exception:
        pass

    # 4) Cleanup & optional Smooth
    try:
    ...
    if SMOOTH_ITER > 0:
        try:
            mesh = mesh.filter_smooth_taubin(number_of_iterations=int(SMOOTH_ITER))
        except Exception:
            mesh = mesh.filter_smooth_simple(number_of_iterations=min(int(SMOOTH_ITER), 2))

    # 5) Farben vom Point-Cloud auf die Mesh-Vertices (nearest neighbor)
    try:
        if len(pcd.colors) > 0:
            ...
            try:
                ...
                # kein scipy -> ohne Farben speichern
                pass
    except Exception:
        pass
    ...
    o3d.io.write_triangle_mesh(
        out_ply, mesh, write_vertex_normals=True, write_vertex_colors=True
    )
    return out_ply
"""
