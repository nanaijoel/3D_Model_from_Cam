...
def _transfer_vertex_colors(mesh, pcd, k=1):
    if not pcd.has_colors(): return mesh
...
def reconstruct_solid_mesh_from_ply(
    ...
    *,
    ...
    if pre_filter:
        pcd, _ = pcd.remove_statistical_outlier(nb_neighbors=max(8, int(pre_filter_neighbors)),
    ...
    if method == "poisson":
        mesh, dens = _mesh_poisson(pcd, depth=int(depth), scale=float(scale), linear_fit=(not bool(no_linear_fit)))
