...
# LightGlue auf gespeicherten DISK-Features  +  LoFTR(gemappt)
# Speichert echte Keypoint-Indexpaare (SfM-kompatibel)
# ==========================================================
import os
...
def _adaptive_tol_px(shape_hw: Tuple[int,int]) -> float:
    ...
    h, w = shape_hw
    return max(3.0, 0.005 * float(max(h, w)))  # z.B. 1080x1920 -> ~9.6 px
...
def _nn_map_one_way(query_xy: np.ndarray, ref_xy: np.ndarray, tol_px: float) -> np.ndarray:
    ...
    for s in range(0, len(query_xy), CH):
        ...
        d2 = q2 + ref2[None, :] - 2.0 * q @ ref_xy.T
...
def _mutual_nn_indices(
    ...
    """
    idx_i = _nn_map_one_way(pts0.astype(np.float32), kps_i, tol_px)
    idx_j = _nn_map_one_way(pts1.astype(np.float32), kps_j, tol_px)
    ...
    # Rückrichtung (leichtgewichtige Identitätsprüfung)
    back_i = _nn_map_one_way(kps_i[idx_i], kps_i, tol_px)
    back_j = _nn_map_one_way(kps_j[idx_j], kps_j, tol_px)
    ok = (back_i == np.arange(len(idx_i), dtype=np.int32)) & \
         (back_j == np.arange(len(idx_j), dtype=np.int32))
    if not np.any(ok):
        return np.empty((0, 2), np.int32)

    pairs = np.stack([idx_i[ok], idx_j[ok]], axis=1).astype(np.int32)
    if len(pairs):
...
def _lg_match_indices_from_saved_features(
    ...
    # Kompatibel mit beiden Output-Varianten
    if "matches0" in out:
...
def build_pairs(
    ...
    - LoFTR liefert zusätzliche Pixel-Matches → per mutual NN + adaptiver Toleranz auf DISK gemappt
    ...
    # Bildliste (reine Reihenfolge); Matching machen wir nur Nachbarn (i,i+1)
    ...
    loftr = LoFTRExtractor(device=device, weights="outdoor")
    ...
    for i in range(len(images) - 1):
        ...
        # ---- LoFTR (Pixel) + Mapping auf DISK-Indices (mutual NN, adaptive tol) ----
        pts0, pts1 = loftr.match_pair(img1, img2, max_size=640)
        ...
        # (Optional) Geometrie-Check auf Pixel-Ebene (robustifizieren)
        # Wir validieren nur, wenn genügend Paare da sind:
        if len(all_pairs) >= 8:
            # Baue Pixelkoordinaten aus DISK-KPs
            p0 = k0[all_pairs[:, 0]].astype(np.float32)
            p1 = k1[all_pairs[:, 1]].astype(np.float32)
            F, inl = cv.findFundamentalMat(p0, p1, cv.FM_RANSAC, 1.5, 0.999)
            if inl is not None:
                inl = inl.ravel() > 0
                all_pairs = all_pairs[inl]
